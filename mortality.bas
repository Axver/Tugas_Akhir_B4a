...
Sub Activity_Create(FirstTime As Boolean)
	...
	SelectedRow = -1
	'add header
	SetHeader(Array As String("Id", "Citizen", "Age", "Cause Of Death"))
	job2.Initialize("Job2", Me)
	
	job2.PostString(domain&"ta_v2/endpoint/mortality.php", "send=test" &"&data=test")
		...
		Select Job.JobName
			Case "Job2"
				...
				For Each colfeatures As Map In features
					...
					Dim date As String = properties.Get("date")
					Dim citizen_id As String = properties.Get("citizen_id")
					Dim cause_date As String = properties.Get("cause_date")
					Dim age As String = properties.Get("age")
					AddRow(Array As String(i, citizen_id, age, cause_date))
					i=i+1
				Next
